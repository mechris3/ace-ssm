...
const engineSliceArb: fc.Arbitrary<EngineSliceState> = engineStateArb.map(s => ({ state: s, activeGoal: null, solutionFocusNodeId: null }));
...
describe('Property 15: Engine FSM Transition Correctness', () => {
  ...
  it('engineStart from IDLE should transition to THINKING', () => {
    fc.assert(
      fc.property(fc.constant({ state: EngineState.IDLE, activeGoal: null, solutionFocusNodeId: null } as EngineSliceState), (prev) => {
  ...
  it('engineInquiry from THINKING should transition to INQUIRY', () => {
    fc.assert(
      fc.property(fc.constant({ state: EngineState.THINKING, activeGoal: null, solutionFocusNodeId: null } as EngineSliceState), (prev) => {
  ...
  it('engineInquiryAnswered from INQUIRY should transition to IDLE', () => {
    fc.assert(
      fc.property(fc.constant({ state: EngineState.INQUIRY, activeGoal: null, solutionFocusNodeId: null } as EngineSliceState), (prev) => {
  ...
  it('invalid transitions should be ignored (state unchanged)', () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom(...nonIdleStates), (s) => {
        const prev: EngineSliceState = { state: s, activeGoal: null, solutionFocusNodeId: null };
        const next = engineReducer(prev, EngineActions.engineStart());
